...
Fully fixed for numeric/categorical preprocessing.
"""
import pandas as pd
import numpy as np
...
class FraudFeatureEngineer:
    ...
    def create_time_based_features(self) -> pd.DataFrame:
        """Engineer time-based features and drop original datetimes."""
        df = self.df.copy()
        ...
        # Drop original datetime columns to avoid errors
        df = df.drop(columns=['signup_time', 'purchase_time'])

        time_features = ['hour_of_day', 'day_of_week', 'is_weekend', 'time_since_signup', 'purchase_month', 'purchase_day']
    ...
    def create_frequency_features(self) -> pd.DataFrame:
        ...
        for user_id in df['user_id'].unique():
            ...
            for idx, row in user_df.iterrows():
                ...
                df.loc[idx, 'transactions_last_1h'] = ((user_df['purchase_time'] >= current_time - timedelta(hours=1)) &
                                                      (user_df['purchase_time'] < current_time)).sum()
                df.loc[idx, 'transactions_last_24h'] = ((user_df['purchase_time'] >= current_time - timedelta(hours=24)) &
                                                       (user_df['purchase_time'] < current_time)).sum()
                df.loc[idx, 'transactions_last_7d'] = ((user_df['purchase_time'] >= current_time - timedelta(days=7)) &
                                                      (user_df['purchase_time'] < current_time)).sum()
    ...
    def create_aggregate_features(self) -> pd.DataFrame:
        ...
        aggregate_features = ['avg_purchase', 'std_purchase', 'max_purchase', 'min_purchase', 'purchase_deviation', 'purchase_zscore']
